...
using Soenneker.Fastly.Runners.OpenApiClient.Utils.Abstract;
...
namespace Soenneker.Fastly.Runners.OpenApiClient.Utils;
...
public sealed class FileOperationsUtil : IFileOperationsUtil
{
    ...
    public FileOperationsUtil(ILogger<FileOperationsUtil> logger, IConfiguration configuration, IGitUtil gitUtil, IDotnetUtil dotnetUtil, IProcessUtil processUtil, 
        IFileDownloadUtil fileDownloadUtil, IFileUtil fileUtil, IDirectoryUtil directoryUtil, IOpenApiFixer openApiFixer, IPostmanConverter postmanConverter)
    {
        _logger = logger;
    ...
    public async ValueTask Process(CancellationToken cancellationToken = default)
    {
        ...
        string fastlyFilePath = Path.Combine(gitDirectory, "fastly.json");

        await _fileUtil.DeleteIfExists(fastlyFilePath, cancellationToken: cancellationToken);

        string openApiDocumentUrl = _configuration["Fastly:ClientGenerationUrl"] ?? "https://www.fastly.com/documentation/downloads/fastly.collection.json";

        await _fileDownloadUtil.Download(openApiDocumentUrl,
            fastlyFilePath, fileExtension: ".json", cancellationToken: cancellationToken);

        string openapi = await _postmanConverter.ConvertFileToJson(fastlyFilePath, cancellationToken);

        string openApiFilePath = Path.Combine(gitDirectory, "openapi.json");

        await _fileUtil.Write(openApiFilePath, openapi, true, cancellationToken);
        ...
        await _processUtil.Start("kiota", gitDirectory, $"kiota generate -l CSharp -d \"{fixedFilePath}\" -o src/{Constants.Library} -c FastlyOpenApiClient -n {Constants.Library}",
